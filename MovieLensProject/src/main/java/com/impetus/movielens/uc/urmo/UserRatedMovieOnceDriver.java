...
public class UserRatedMovieOnceDriver extends MyJobConfiguration {
	public static void main(String[] args) {
		System.out.println("USer Id who has rated movie atlease once..!");

		int exitCode = 0;
		try {
			exitCode = ToolRunner.run(configuration,
					new UserRatedMovieOnceDriver(), args);
	...
	public int run(String[] runArgs) throws Exception {
		...
		// /user/ubuntu/mgp/MovieLens/output/op/urmo
		// runArgs[0] will be the class name that you would give in the cli
		String in = runArgs[0];
		String out = runArgs[1]; // /user/ubuntu/mgp/MovieLens/output/op/urmo
	...
	private Job getJobConfig() throws Exception {
		MyJobDefinition jobDefinition = new MyJobDefinition() {
			...
			public Class<? extends Reducer> getMyReducerClass() {
				...
				return UserRatingMovieOnceReducer.class;
			}
			...
			public Class<? extends Mapper> getMyMapperClass() {
				// TODO Auto-generated method stub
				return UserRatingMovieOnceMapper.class;
			}
		...
		Job job = setupJob("User Id rated atleast once for any movie", jobDefinition);
		return job;
	}
}
