...
public class MovieRatedOnceDriver extends MyJobConfiguration {
	public static void main(String[] args) {
		System.out.println("Movie id s that has been rated atleast once..!");

		int exitCode = 0;
		try {
			exitCode = ToolRunner.run(configuration,
					new MovieRatedOnceDriver(), args);
	...
	public int run(String[] runArgs) throws Exception {
		...
		// /user/ubuntu/mgp/MovieLens/output/op/mro
		// runArgs[0] will be the class name that you would give in the cli
		String in = runArgs[0];
		String out = runArgs[1]; // /user/ubuntu/mgp/MovieLens/output/op/mro
	...
	private Job getJobConfig() throws Exception {
		MyJobDefinition jobDefinition = new MyJobDefinition() {
			...
			public Class<? extends Reducer> getMyReducerClass() {
				...
				return MovieRatedOnceReducer.class;
			}
			...
			public Class<? extends Mapper> getMyMapperClass() {
				// TODO Auto-generated method stub
				return MovieRatedOnceMapper.class;
			}
		...
		Job job = setupJob("Movie Id rated atleast once ", jobDefinition);
		return job;
	}
}
